...
from app.api.routes import client, messages, company, auth, warnings
...
def init_router(api: Api, server: Flask):
    ...
    api.add_resource(messages.SpeechResource, "/speeches")
    api.add_resource(messages.MessageResource, "/messages")
    api.init_app(server)
